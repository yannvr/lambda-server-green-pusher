#!/bin/sh
{
    ...
    # Warm up function
    ret=`curl -H "cypress: 42" -H "meonly: 1" https://greenlife-quote-pusher.yannvr.now.sh/api/warmup`
    ...
    if [ $ret = "HOT" ] ; then
        ...
        ret=`curl -H "cypress: 42" -H "meonly: 1" https://greenlife-quote-pusher.yannvr.now.sh/api/broadcast-quote`
        echo "PUSH ret: ${ret}"
        if [ $ret = "OK" ] ; then
            echo "${date}: OK"
        else
